...
> 단어 기반으로 모든 식별자를 통제하는 자동화된 용어 사전 시스템
...
3. [시스템 구조](#3-시스템-구조)
4. [핵심 개념](#4-핵심-개념)
5. [디렉토리 및 파일 구조](#5-디렉토리-및-파일-구조)
6. [서브모듈 설치 및 설정](#6-서브모듈-설치-및-설정)
...
│   │  bin/scan_items.py           ← 소스 스캔              │  │
│   │  bin/batch_items.py          ← API 추출 및 판단       │  │
│   └─────────────────────────────────────────────────────┘  │
│                                                             │
│   ┌───────────────────────────────────────────────────┐    │
│   │  프로젝트 소스 (ts/)                               │    │
...
```
[소스 코드 스캔]          [API 분석 및 판단]      [사전 등록]
scan_items.py    →    batch_items.py    →    웹 UI 검토 후
...
│   ├── scan_items.py               # 소스 스캔 → 용어 후보 추출
│   ├── batch_items.py              # 분석 파이프라인 → items_날짜.json 생성
│   └── enrich_items.py             # 다국어 및 설명 누락 자동 보완 유틸리티
│
├── web/                            # 웹 관리 UI
...
### 사전 정보 자동 보완 (enrich_items)

`words.json`에 반자동으로 추가된 단어들 중, 누락된 다국어(`ja`, `zh_hans`, `ko`), 설명(`description_i18n.ko`), 품사(`canonical_pos`) 정보를 외부 API를 경유하여 일괄적으로 되채웁니다(Backfill).

**3대 핵심 원칙:** "사전 우선(Dictionary First), AI 보완(AI Fallback), 기존 값 보호(Non-destructive)"
...
## 10. 일괄 추출 (batch_items)

소스 코드 전체를 스캔해서 미등록 용어 후보를 분류하는 자동화 도구입니다.

### 실행 순서

```
1. 스캔 (API 없이)
   python glossary/bin/scan_items.py --count     # 후보 수 확인
   python glossary/bin/scan_items.py             # 상세 목록

2. 실제 분석 (모드 선택)
   python glossary/bin/batch_items.py --mode word --register-mode auto
   python glossary/bin/batch_items.py --mode word --register-mode safety
   python glossary/bin/batch_items.py --mode word --register-mode normal

3. 결과 확인
   프로젝트루트/tmp/items/items_날짜.json
...
- **CLI Auto 모드**: CLI에서 `auto` 모드로 실행 시 승인 절차 없이 즉시 `words.json`에 병합 및 저장됩니다.
- **웹 UI 사용**: 결과 검토 후 반영이 필요하다면 반드시 웹 UI의 `Auto` 또는 `Safety` 모드를 사용하세요.
- **Auto 모드 신규 단어 정책**: 일반 영어 단어는 원칙적으로 등록을 허용하며, 반복 사용되는 일반 단어는 blacklist로 제외하지 않습니다. 단, 다음은 노이즈로 간주해 등록을 자동 제외합니다.
  - 길이 1~2의 토큰
  - 숫자 포함 토큰
  - 의미 없는 축약 조각
  - 식별자 분해 중 생성된 비의미 조각
...
A. 기존 코드는 소급 적용하지 않습니다. 신규 식별자 생성 시부터 적용하면 됩니다. 배치 추출 도구(`batch_items.py`)로 일괄 정리할 수 있습니다.
...
A. `tmp/items/items_날짜.json` 파일을 열어 API 응답 원문을 확인하세요. JSON이 잘렸다면 `.env`의 `MAX_OUTPUT_TOKENS`를 늘리세요 (권장: 8192).
