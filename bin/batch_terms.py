...
12. 추출 대상 용어가 복합어(2개 이상의 단어 결합)인 경우, 반드시 `is_compound: true`로 설정하고 구성 기본 단어들을 `base_words` 배열에 포함시킨다.
    - `base_words`의 각 원소는 `id`(단수형), `ko`(한글 뜻, 사전 및 주석 참고), `description`(프로그램 등록 위치/클래스 기반 기능 설명), `plural`(단어가 복수형에서 유래했다면 그 원문 문자열, 아니면 null) 필드를 가진다.
...
[
  {{
    ...
    "plural": null,
    "base_words": [
      {{"id": "snake", "ko": "뱀", "description": "Snake 클래스/객체 정보", "plural": "snakes"}}
    ],
