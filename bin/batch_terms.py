...
5. 자동 약어 생성 금지. terms에서 임의로 abbreviation 필드를 생성하지 마세요.
...
12. 추출 대상 용어가 복합어(2개 이상의 단어 결합)인 경우, 반드시 `is_compound: true`로 설정하고 구성 기본 단어들을 `base_words` 배열에 포함시킨다.
    - `base_words`의 각 원소는 `id`(단수형), `lang`(`{"ko": "한글 뜻"}`, 사전 및 주석 참고), `description_i18n`(`{"ko": "설명"}`), `plural`(단어가 복수형에서 유래했다면 그 원문 문자열, 아니면 null) 필드를 가진다.
13. 단일어인 경우 `is_compound: false`로 설정하고 `plural` 필드(복수형 원문 or null)를 제공한다.
14. 검토 시 논의나 사람이 봐야 할 특이점이 있다면 `issues_comment` 속성에 기록한다."""
...
[
  {{
    "id": "snake_case_id",
    "lang": {{"ko": "한글명", "en": "English Name"}},
    "domain": "category1",
    "description_i18n": {{"ko": "설명"}},
    "is_compound": true,
    "plural": null,
    "base_words": [
      {{"id": "snake", "lang": {{"ko": "뱀"}}, "description_i18n": {{"ko": "Snake 클래스/객체 정보"}}, "plural": "snakes"}}
    ],
    "issues_comment": "AI가 인간에게 남기는 리뷰 노트"
  }}
]"""
...
def main():
    ...
    parser.add_argument("--ui-prog", action="store_true", help="UI 파싱용 PROGRESS 로그 출력")
    ...
    print(f"      후보 {cand_len}개 추출됨")
    prog(1, "소스 스캔", cand_len, cand_len, "완료")
    ...
    for i in range(n_chunks):
        prog(2 + i, f"API 청크 {i+1}/{n_chunks}", 0, len(chunks[i]), "대기 중")
    ...
    with open(raw_log, 'w', encoding='utf-8') as raw_f:
        for i, (chunk, prompt) in enumerate(zip(chunks, prompts), 1):
            clen = len(chunk)
            print(f"      청크 {i}/{n_chunks} 처리 중 ({clen}개)...", flush=True)
            prog(1 + i, f"API 청크 {i}/{n_chunks}", clen, clen, "진행중")
            try:
                response = call_api(prompt, env, max_tokens)
                parsed   = parse_response(response)
                all_terms.extend(parsed)
                print(f"→ {len(parsed)}개 용어 추출")
                prog(1 + i, f"API 청크 {i}/{n_chunks}", clen, clen, "완료")
                ...
                err_msg = str(e)
                print(f"→ 오류: {err_msg}")
                prog(1 + i, f"API 청크 {i}/{n_chunks}", clen, clen, "오류")
                raw_f.write(f"\n=== 청크 {i} 오류 ===\n{err_msg}\n")
                raw_f.flush()
